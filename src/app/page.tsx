...
interface AgentRT{
  def:AgentDef;si:number;ax:number;ay:number;
  tileX:number;tileY:number;path:{x:number,y:number}[];
  state:AState;dir:Dir;animT:number;animF:number;
  seatI:number;bubble:string|null;bubbleT:number;
  actionT:number;returning:boolean;
}
...
function drawScene(ctx:CanvasRenderingContext2D,agents:AgentRT[],sprites:(HTMLImageElement|null)[],fc:number){
  ...
  ctx.fillRect(0,0,W,RT);ctx.fillRect(0,H-RT,W,RT);
  ctx.fillRect(0,0,RT,H);ctx.fillRect(W-RT,0,RT,H);
  for(const wc of [3,7,11,15]){
    const wx=wc*RT,wy=4;
    ctx.fillStyle="#0d1b4e";ctx.fillRect(wx,wy,RT*2,RT-8);
    ctx.strokeStyle="#3a2a7e";ctx.lineWidth=2;ctx.strokeRect(wx,wy,RT*2,RT-8);
    ...
    for(const [sx,sy] of [[8,8],[20,15],[30,5],[42,18],[55,10],[15,28],[38,30],[60,22]])ctx.fillRect(wx+sx,wy+sy,1,1);
    ctx.globalAlpha=1;
    ctx.fillStyle="#fffde7";ctx.beginPath();ctx.arc(wx+RT*2-10,wy+10,6,0,Math.PI*2);ctx.fill();
    ctx.fillStyle="#0d1b4e";ctx.beginPath();ctx.arc(wx+RT*2-8,wy+8,5,0,Math.PI*2);ctx.fill();
  }
  for(let r=1;r<ROWS-1;r++)for(let c=1;c<COLS-1;c++){
    ctx.fillStyle=(c+r)%2===0?"#16213e":"#1a1a2e";ctx.fillRect(c*RT,r*RT,RT,RT);
  }
  ...
  for(let i=0;i<10;i++){
    const seat=SEATS[i],isTop=i<5;
    const deskTy=isTop?seat.ty-1:seat.ty+1;
    const dsx=seat.tx*RT-RT,dsy=deskTy*RT;
    ctx.fillStyle="#2a1f3d";ctx.fillRect(dsx,dsy,RT*3,RT);
    ctx.strokeStyle="#4a3f6d";ctx.lineWidth=1;ctx.strokeRect(dsx,dsy,RT*3,RT);
    const mx=seat.tx*RT+4,my=dsy+4,mw=RT-8,mh=RT-10;
    rr(ctx,mx,my,mw,mh,3);ctx.fillStyle="#111827";ctx.fill();ctx.strokeStyle="#374151";ctx.lineWidth=1;ctx.stroke();
    ctx.fillStyle="#0d1117";ctx.fillRect(mx+3,my+3,mw-6,mh-8);
    ...
    for(let li=0;li<4;li++){
      ...
      ctx.fillStyle=lc[li%4];ctx.fillRect(mx+5,my+5+li*6,lw,2);
    }
    ctx.globalAlpha=1;
    if(Math.floor(fc/25)%2===0){ctx.fillStyle="#fff";ctx.fillRect(mx+5,my+mh-10,2,3);}
    ctx.fillStyle="#1f2937";ctx.fillRect(dsx+RT/2,dsy+RT*0.65,RT*2,RT*0.25);
    ctx.strokeStyle="#374151";ctx.lineWidth=0.5;ctx.strokeRect(dsx+RT/2,dsy+RT*0.65,RT*2,RT*0.25);
    const ag=agents[i];
    if(ag){
      ctx.font="bold 9px monospace";
      const label=ag.def.name+` (${ag.def.role})`,tw=ctx.measureText(label).width;
      const bx2=seat.tx*RT+RT/2-tw/2-4,by2=isTop?seat.ty*RT+RT+2:seat.ty*RT-16;
      rr(ctx,bx2,by2,tw+8,13,2);
      ctx.fillStyle="#0f0f1f";ctx.fill();ctx.strokeStyle=ag.def.shirtColor;ctx.lineWidth=1;ctx.stroke();
      ctx.fillStyle=ag.def.shirtColor;ctx.fillText(label,bx2+4,by2+10);
    }
  }
  for(const [cx,cy] of [[1,1],[COLS-2,1],[1,ROWS-2],[COLS-2,ROWS-2]] as [number,number][]){
    const px=cx*RT+RT/2,py=cy*RT+RT-4;
    ctx.fillStyle="#7c2d12";rr(ctx,px-10,py-12,20,12,2);ctx.fill();ctx.strokeStyle="#9a3412";ctx.lineWidth=1;ctx.stroke();
    ctx.fillStyle="#15803d";ctx.fillRect(px-2,py-28,4,16);
  ...
  for(const ag of sorted){drawChar(ctx,sprites,ag,Math.round(ag.ax*SCALE),Math.round(ag.ay*SCALE));}
  for(const ag of agents){if(ag.bubble)drawBubble(ctx,ag,ag.bubble,Math.round(ag.ax*SCALE),Math.round(ag.ay*SCALE));}
}

function drawChar(ctx:CanvasRenderingContext2D,sprites:(HTMLImageElement|null)[],ag:AgentRT,sx:number,sy:number){
  ...
  if(sprite&&sprite.complete&&sprite.naturalWidth>0){
    ...
    const S=SCALE,{def,state,animF}=ag,px=sx-8*S,py=sy-32*S;
    const lOff=state==="walk"?Math.sin(animF*1.5)*2*S:0;
    ctx.fillStyle="#1e293b";ctx.fillRect(px+4*S,py+26*S+lOff,3*S,5*S);ctx.fillRect(px+9*S,py+26*S-lOff,3*S,5*S);
    ctx.fillStyle="#111";ctx.fillRect(px+3*S,py+31*S+lOff,5*S,2*S);ctx.fillRect(px+8*S,py+31*S-lOff,5*S,2*S);
    ctx.fillStyle=def.shirtColor;ctx.fillRect(px+2*S,py+14*S,12*S,12*S);
    const aOff=state==="walk"?Math.sin(animF*1.5)*2*S:state==="type"?Math.sin(animF*3)*S:0;
    ctx.fillRect(px,py+15*S+aOff,2*S,8*S);ctx.fillRect(px+14*S,py+15*S-aOff,2*S,8*S);
    ctx.fillStyle="#d4a574";ctx.fillRect(px+4*S,py+5*S,8*S,10*S);
    ctx.fillStyle=def.hairColor;ctx.fillRect(px+4*S,py+5*S,8*S,3*S);
    ctx.fillRect(px+4*S,py+8*S,2*S,2*S);ctx.fillRect(px+10*S,py+8*S,2*S,2*S);
    ctx.fillStyle="#fff";ctx.fillRect(px+5*S,py+9*S,2*S,2*S);ctx.fillRect(px+9*S,py+9*S,2*S,2*S);
    ctx.fillStyle="#000";ctx.fillRect(px+6*S,py+10*S,1*S,1*S);ctx.fillRect(px+10*S,py+10*S,1*S,1*S);
    ctx.fillStyle="#7f1d1d";ctx.fillRect(px+6*S,py+12*S,4*S,state==="idle"?2*S:1*S);
  }
}
...
function drawBubble(ctx:CanvasRenderingContext2D,ag:AgentRT,text:string,sx:number,sy:number){
  ...
  const charTop=sy-CHAR_H*SCALE,by=Math.max(2,charTop-bh-8);
  rr(ctx,bx,by,bw,bh,4);ctx.fillStyle="#0f0f1f";ctx.fill();
...
export default function Home(){
  ...
  useEffect(()=>{
    agentsRef.current=AGENTS.map((def,i)=>{
      const seat=SEATS[i],ax=seat.tx*TILE+TILE/2,ay=seat.ty*TILE+TILE/2;
      return{def,si:i%6,ax,ay,tileX:seat.tx,tileY:seat.ty,path:[],
        state:"type" as AState,dir:seat.dir,animT:0,animF:0,seatI:i,
        bubble:null,bubbleT:0,actionT:5+Math.random()*15,returning:false};
    });
  },[]);
    ...
    function updateAgent(ag:AgentRT,dt:number){
      ...
      if(ag.path.length>0){
        ...
        if(dist<=step+0.01){
          ...
          if(ag.path.length===0){
            if(ag.returning){
              ag.returning=false;ag.state="type";ag.dir=SEATS[ag.seatI].dir;
              ag.animF=0;ag.actionT=10+Math.random()*20;
            }else{ag.state="idle";ag.animF=0;}
          }
      ...
      if(ag.state==="idle"){
        ag.actionT-=dt;
        if(ag.actionT<=0){
          const seat=SEATS[ag.seatI],path=bfs(ag.tileX,ag.tileY,seat.tx,seat.ty);
        ...
        if(ag.actionT<=0){
          if(Math.random()<0.4){
            ...
            for(let dy2=-5;dy2<=5;dy2++)for(let dx2=-5;dx2<=5;dx2++){
              const nx=ag.tileX+dx2,ny=ag.tileY+dy2;
              if(nx>0&&ny>0&&nx<COLS-1&&ny<ROWS-1&&WALKABLE[ny][nx])cands.push({x:nx,y:ny});
            }
            if(cands.length>0){
              const tgt=cands[Math.floor(Math.random()*cands.length)];
              const path=bfs(ag.tileX,ag.tileY,tgt.x,tgt.y);
              if(path.length>0){ag.path=path;ag.state="walk";ag.returning=false;ag.actionT=3+Math.random()*5;}
                ...
                setTimeout(()=>{
                  if(ag.state==="idle"||ag.path.length===0){
                    ...
                    setTimeout(()=>{
                      ...
                      setTimeout(()=>{
                        const seat=SEATS[ag.seatI],back=bfs(ag.tileX,ag.tileY,seat.tx,seat.ty);
  ...
  async function sendMessage(){
    ...
    setChatHistories(p=>({...p,[selectedId!]:[...history,{role:"user" as const,text:msg}]}));
    try{
        ...
        {selAgent&&(
          <div style={{width:300,display:"flex",flexDirection:"column",background:"#0c0c24",borderLeft:`3px solid ${selAgent.shirtColor}`,flexShrink:0}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:10,display:"flex",flexDirection:"column",gap:7}}>
              {chatHist.length===0&&<div style={{color:"#334155",fontSize:10,textAlign:"center",marginTop:30}}>Start a conversation with {selAgent.name}</div>}
              {chatHist.map((m,i)=>(
                <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                  <div style={{maxWidth:"82%",padding:"5px 9px",borderRadius:7,fontSize:11,lineHeight:1.45,
                    ...
                    color:m.role==="user"?"#bfdbfe":"#e2e8f0"}}>{m.text}</div>
                </div>
              ))}
            </div>
