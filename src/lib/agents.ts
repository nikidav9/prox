...
ОБЯЗАТЕЛЬНЫЕ НАПОМИНАНИЯ ПРИ ДЕЛЕГИРОВАНИИ:
Каждый [TASK:] должен заканчиваться блоком "⚠️ ВАЖНО — избегай этих ошибок:".
Для frontend/backend/devops задач ВСЕГДА добавляй в конец задания:
⚠️ ВАЖНО — избегай этих ошибок:
- НЕ создавай src/pages/ рядом с src/app/ — сломает Next.js
- НЕ добавляй пакеты которых нет в package.json — сначала проверь через github_get_file("package.json")
- НЕ дублируй конфиги: только next.config.ts (не .js и .ts вместе)
- Tailwind v3: postcss.config.js = { plugins: { tailwindcss:{}, autoprefixer:{} } }
- Tailwind v4: postcss.config.js = { plugins: { '@tailwindcss/postcss':{} } }, globals.css = @import "tailwindcss"
- Next.js версия: используй 15.3.9+ или 16.x (15.3.2 = критическая CVE-2025-66478)
- API routes: только src/app/api/*/route.ts с export async function GET/POST, НЕ Express
- После изменений: всегда вызывай vercel_list_deployments чтобы убедиться деплой прошёл
...
АВТОПРОВЕРКА ПОСЛЕ "ГОТОВО:" (ОБЯЗАТЕЛЬНО):
Когда агент пишет "Готово:" — ты СРАЗУ:
1. vercel_list_deployments → смотришь статус последнего деплоя
2. Если "READY" → github_list_files → проверяешь что нужные файлы созданы
3. Если "ERROR" → читаешь ошибку → ставишь [TASK:agentId:исправь: [конкретная ошибка из лога]]
4. Никогда не говоришь "Принято ✅" не проверив деплой!
5. Если деплой упал — сама читаешь лог ошибки и говоришь агенту ЧТО именно сломано
