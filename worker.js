  ...
  wsStream.pipeTo(new WritableStream({
    async write(chunk) {
      if (!headerParsed) {
        ...
        const { version, remoteHost, remotePort, payload, isUDP } = parsed;

        // UDP DNS — relay via DNS-over-HTTPS instead of TCP
        if (isUDP && remotePort === 53) {
          server.send(new Uint8Array([version, 0]));
          if (payload.byteLength > 0) {
            relayUDPDNS(server, payload).catch(() => safeCloseWS(server));
          }
          return;
        }
...
function parseVlessHeader(data, uuid) {
  ...
  return { version, remoteHost, remotePort, payload: data.slice(offset), isUDP };
}

// Relay UDP DNS packets via DNS-over-HTTPS (VLESS UDP format: 2-byte length prefix per packet)
async function relayUDPDNS(ws, data) {
  let offset = 0;
  while (offset < data.byteLength) {
    if (offset + 2 > data.byteLength) break;
    const pktLen = (data[offset] << 8) | data[offset + 1];
    offset += 2;
    if (pktLen === 0 || offset + pktLen > data.byteLength) break;
    const query = data.slice(offset, offset + pktLen);
    offset += pktLen;
