...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET') {
      if (url.pathname === '/pac') {
        return new Response(makePAC(host, PROXY_USER, PROXY_PASS), {
...
function makePAC(host, user, pass) {
  return `// PAC — prox on Cloudflare Workers
...
.btn{display:block;width:100%;padding:16px;background:#007AFF;color:white;text-decoration:none;
  border-radius:12px;text-align:center;font-size:17px;font-weight:600;margin-bottom:16px}
...
li::before{content:counter(s);position:absolute;left:0;top:8px;background:#007AFF;color:white;
...
<a href="${base}/profile.mobileconfig" class="btn">Скачать iOS профиль (.mobileconfig)</a>
<div class="c"><h2>Установка на iPhone</h2>
<ol>
  <li>Открой эту страницу на iPhone в Safari</li>
  <li>Нажми «Скачать iOS профиль»</li>
  <li>Настройки → Общие → VPN и управление устройством</li>
  <li>Профиль «prox» → Установить</li>
  <li>Готово — весь трафик через прокси</li>
</ol>
<p class="n">Без App Store. Без аккаунта разработчика. Работает на Wi-Fi и мобильной сети.</p>
</div></div></body></html>`;
}
